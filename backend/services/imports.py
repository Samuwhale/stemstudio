...
from backend.core.constants import SUPPORTED_IMPORT_EXTENSIONS, SUPPORTED_IMPORT_EXTENSIONS_HINT
...
_DRAFT_TEXT_MAX_LENGTH = 255
_DRAFT_FILENAME_MAX_LENGTH = 255
_UNSET = object()


def _bounded_text(value: str | None, *, fallback: str, max_length: int = _DRAFT_TEXT_MAX_LENGTH) -> str:
...
def _bounded_optional_text(value: str | None, *, max_length: int = _DRAFT_TEXT_MAX_LENGTH) -> str | None:
    cleaned = re.sub(r"\s+", " ", value or "").strip()
    if not cleaned:
        return None
...
def _safe_import_filename(value: str | None, *, fallback: str = "untitled") -> str:
    cleaned = re.sub(r"[\x00-\x1f]+", " ", value or "").strip()
    cleaned = cleaned.replace("\\", "/").rsplit("/", 1)[-1].strip() or fallback
    if len(cleaned) <= _DRAFT_FILENAME_MAX_LENGTH:
        return cleaned

    path = Path(cleaned)
    suffix = path.suffix
    if len(suffix) >= _DRAFT_FILENAME_MAX_LENGTH:
        suffix = suffix[-(_DRAFT_FILENAME_MAX_LENGTH - 1):]
    stem_limit = max(1, _DRAFT_FILENAME_MAX_LENGTH - len(suffix))
...
def resolve_youtube_import(
    ...
    for item in resolved.items:
        ...
        title = _bounded_text(item.title, fallback="Untitled Track")
        artist = _bounded_optional_text(item.artist)
        draft = ImportDraft(
...
def resolve_local_import(
    ...
    try:
        for original_name, file_handle in files:
            display_name = _safe_import_filename(original_name)
            extension = Path(display_name).suffix.lower()
            if extension not in SUPPORTED_IMPORT_EXTENSIONS:
                raise ValueError(
                    f"Unsupported file type '{extension or 'unknown'}' for '{display_name}'. "
                    f"{SUPPORTED_IMPORT_EXTENSIONS_HINT}"
                )
            ...
            if first_name is not None:
                raise ValueError(
                    f"'{display_name}' is the same audio file as '{first_name}'. "
                    "Remove duplicate files and try again."
                )
            seen_content_hashes[content_hash] = display_name
...
def confirm_import_drafts(
    ...
    if not payload.draft_ids:
        raise ConfirmValidationError("Select at least one song to import.")
    ...
    if payload.queue:
        if not is_worker_alive(runtime_settings):
            raise WorkerUnavailableError("Worker is offline. Restart StemStudio before queueing stem creation.")
        application_settings = get_or_create_settings(session, runtime_settings)
        resolve_storage_paths(runtime_settings, application_settings)
...
def _apply_draft_patch(
    ...
    *,
    ...
    if title is not _UNSET:
        ...
        cleaned = _bounded_text(title, fallback="")
        if not cleaned:
            raise ValueError("Title cannot be empty.")
        draft.title = cleaned
    if artist is not _UNSET:
        draft.artist = _bounded_optional_text(artist if isinstance(artist, str) else None)
...
class WorkerUnavailableError(RuntimeError):
    """Raised when a request needs the background worker but it is offline."""
...
def _restore_pending_file(source_path: Path, target_path: Path) -> None:
    ...
    if target_path.exists():
        raise FileExistsError(f"Cannot restore staged import; target already exists: {target_path}")
...
def _title_from_filename(filename: str) -> str:
    ...
    return _bounded_text(stem, fallback="Untitled Track")
...
def serialize_draft(session: Session, draft: ImportDraft) -> ImportDraftResponse:
    ...
    return ImportDraftResponse(
        ...
        duration_seconds=_optional_finite_float(draft.duration_seconds),
        original_filename=draft.original_filename,
        content_hash=draft.content_hash,
        size_bytes=_optional_nonnegative_int(draft.size_bytes),
