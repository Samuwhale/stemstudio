...
UNSET = object()
MIX_CACHE_ARTIFACT_KINDS = frozenset({"export-mix-wav", "export-mix-mp3"})
TRACK_TEXT_MAX_LENGTH = 255


def _bounded_text(value: str, *, fallback: str, max_length: int = TRACK_TEXT_MAX_LENGTH) -> str:
    cleaned = re.sub(r"\s+", " ", value).strip()
    if not cleaned:
        cleaned = fallback
    return cleaned[:max_length].rstrip() or fallback[:max_length]


def _bounded_filename(value: str, *, fallback: str, max_length: int = TRACK_TEXT_MAX_LENGTH) -> str:
    ...
    if len(suffix) >= max_length:
        suffix = suffix[-(max_length - 1):]
    stem_limit = max(1, max_length - len(suffix))
    return f"{path.stem[:stem_limit].rstrip() or fallback[:stem_limit]}{suffix}"
...
def track_output_root(track: Track, outputs_dir: Path) -> Path:
    ...
    source_slug = metadata_dict(track.metadata_json).get("source_slug") or track.id
...
def progress_value(value: Any) -> float:
    progress = _optional_finite_float(value)
    if progress is None:
        return 0.0
...
def _serialize_artifact_metrics(metrics: dict[str, Any] | None) -> ArtifactMetricsResponse | None:
    ...
    return ArtifactMetricsResponse(
        duration_seconds=_optional_nonnegative_float(metrics.get("duration_seconds")),
        sample_rate=_optional_nonnegative_int(metrics.get("sample_rate")),
        channels=_optional_nonnegative_int(metrics.get("channels")),
        size_bytes=_optional_nonnegative_int(metrics.get("size_bytes")),
        integrated_lufs=_optional_finite_float(metrics.get("integrated_lufs")),
        true_peak_dbfs=_optional_finite_float(metrics.get("true_peak_dbfs")),
        peaks=_finite_float_list(metrics.get("peaks")),
    )
...
def serialize_run_summary(run: Run) -> RunSummaryResponse:
    return RunSummaryResponse(
        ...
        status=run.status,
        progress=progress_value(run.progress),
...
def _invalidate_cached_mix_artifacts(session: Session, run: Run) -> None:
    stale_artifacts = [
        artifact
        for artifact in list(run.artifacts)
        if artifact.kind in MIX_CACHE_ARTIFACT_KINDS
    ]
    if not stale_artifacts:
        return

    schedule_path_cleanup(session, *(Path(artifact.path) for artifact in stale_artifacts))
...
def _is_default_stem(entry: dict[str, Any] | RunMixStemEntry) -> bool:
    ...
    try:
        ...
        gain_value = 0.0
    return abs(gain_value) < 0.01 and not bool(muted)


def _mix_gain_from_raw(value: Any) -> float:
    try:
        gain = float(value or 0.0)
    except (TypeError, ValueError):
        gain = 0.0
    return max(MIX_GAIN_DB_MIN, min(MIX_GAIN_DB_MAX, gain))
    ...
    if isinstance(stems_raw, list):
        for entry in stems_raw:
            ...
            if artifact_id not in visible_ids:
                continue
            gain = _mix_gain_from_raw(entry.get("gain_db"))
            stems.append(
                RunMixStemEntry(
...
def serialize_run_detail(run: Run) -> RunDetailResponse:
    return RunDetailResponse(
        **serialize_run_summary(run).model_dump(),
        metadata_json=metadata_dict(run.metadata_json),
...
def set_run_mix(session: Session, track_id: str, run_id: str, payload: RunMixInput) -> Run:
    ...
    _invalidate_cached_mix_artifacts(session, run)
    _touch_track(track)
    session.commit()
    session.refresh(run)
    return run
    ...
    for run in runs:
        for artifact in run.artifacts:
            if artifact.kind != "source":
                continue
            metrics = metadata_dict(artifact.metrics_json)
            peaks = _finite_float_list(metrics.get("peaks"))
            if peaks:
                return peaks
    return []
...
def serialize_track_summary(track: Track) -> TrackSummaryResponse:
    ...
    return TrackSummaryResponse(
        ...
        duration_seconds=_optional_nonnegative_float(track.duration_seconds),
...
def serialize_track_detail(track: Track) -> TrackDetailResponse:
    return TrackDetailResponse(
        ...
        duration_seconds=_optional_nonnegative_float(track.duration_seconds),
        metadata_json=metadata_dict(track.metadata_json),
        created_at=track.created_at,
        updated_at=track.updated_at,
...
def backfill_content_hashes(session: Session) -> int:
    ...
    for track in session.scalars(select(Track)):
        metadata = dict(metadata_dict(track.metadata_json))
...
def create_track(
    ...
    *,
    ...
    clean_title = _bounded_text(title, fallback="Untitled Track")
    clean_artist = _bounded_text(artist, fallback="", max_length=TRACK_TEXT_MAX_LENGTH) if artist else None
    clean_source_filename = _bounded_filename(source_filename, fallback=source_path.name)
    metadata = {
...
def set_run_state(
    ...
    *,
    ...
    run.progress = progress_value(progress)
...
def recover_orphaned_runs(session: Session) -> int:
    ...
    for run in orphaned:
        if is_cancellation_requested(run):
...
def is_cancellation_requested(run: Run) -> bool:
    metadata = metadata_dict(run.metadata_json)
...
def _measure_paths(paths: Iterable[Path]) -> int:
    unique_paths = {path.absolute() for path in paths if path.exists() or path.is_symlink()}
    return sum(_path_size(path) for path in unique_paths)


def _run_file_paths(run: Run, *, include_source: bool) -> set[Path]:
    paths: set[Path] = set()
    output_root = Path(run.output_directory).absolute() if run.output_directory else None
    if output_root is not None:
        paths.add(output_root)
    for artifact in run.artifacts:
        if not include_source and artifact.kind == "source":
            continue
        artifact_path = Path(artifact.path).absolute()
