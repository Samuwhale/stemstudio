...
def download_track_source(track_id: str, session: Session = Depends(get_db_session)) -> FileResponse:
    ...
    try:
        ...
        raise local_file_http_error("Source download", error) from error
    return FileResponse(
        path=source_path,
...
def create_track_run(
    ...
    if not is_worker_alive(runtime_settings):
        raise HTTPException(
            status_code=503,
            detail="Worker is offline. Restart StemStudio before queueing stem creation.",
        )
    ...
    try:
        ...
        rollback_session(session)
        raise local_file_http_error("Queue stem creation", error) from error
...
def retry_run_endpoint(
    ...
    try:
        resolve_storage_paths(runtime_settings, get_or_create_settings(session, runtime_settings))
    except ValueError as error:
        raise HTTPException(status_code=400, detail=str(error)) from error
    except OSError as error:
        raise local_file_http_error("Retry stem creation", error) from error
