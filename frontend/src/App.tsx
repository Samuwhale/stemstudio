...
import { filterImportableMediaFiles, IMPORTABLE_MEDIA_HINT, unsupportedImportableMediaMessage } from './importableMedia'
...
const DEFAULT_SONGS_VIEW = parseSongsView(new URLSearchParams())
...
function App() {
  ...
  const navigationSongsView = navigationState?.songsView
  const rememberedSongsView = useMemo(
    () => navigationSongsView ?? DEFAULT_SONGS_VIEW,
    [navigationSongsView],
  )
  ...
  const setupIssue = diagnostics?.issues[0]?.trim() || null
  const setupRequired = hasFirstSync && diagnostics
    ? !diagnostics.app_ready || !diagnostics.separation_ready || diagnostics.issues.length > 0
    : false
  const setupReadyForStemCreation = diagnostics
    ? diagnostics.separation_ready && diagnostics.issues.length === 0
    : true
  const stemCreationReady = setupReadyForStemCreation && workerOnline
  const stemCreationBlockedReason = !setupReadyForStemCreation
    ? setupIssue ?? 'Finish setup before creating stems.'
    : 'Restart StemStudio to relaunch the worker before creating stems.'
  ...
  useEffect(() => {
    if (connection.state === 'ready' || connection.state === 'syncing') {
      lastConnectionToastRef.current = null
      return
    }
    if (connection.consecutiveFailures !== 1) return

    const message = connection.lastError ?? 'StemStudio could not sync with the local API.'
    const toastKey = `${connection.state}:${message}`
    if (lastConnectionToastRef.current === toastKey) return

    lastConnectionToastRef.current = toastKey
    pushToast('error', message)
  }, [connection.consecutiveFailures, connection.lastError, connection.state, pushToast])
  ...
  useEffect(() => {
    ...
    function onDrop(event: DragEvent) {
      ...
      const files = filterImportableMediaFiles(droppedFiles)
      const ignoredFileCount = droppedFiles.length - files.length
      if (files.length) {
        discardRejection(async () => {
          await handleResolveLocalImport(files)
          if (ignoredFileCount > 0) {
            pushToast('info', unsupportedImportableMediaMessage(ignoredFileCount))
          }
          if (mixActive) {
      ...
      pushToast('error', `Drop supported audio or video files. ${IMPORTABLE_MEDIA_HINT}`)
    }
  ...
  useKeyboardShortcuts({
    ...
    onEscape: () => {
      ...
      if (importPanelOpen) {
        if (resolvingYoutubeImport || resolvingLocalImport || confirmingDrafts) return
            ...
            <div className="app-top-actions">
              ...
              {!workerOnline ? (
                <button
                  type="button"
                  className="topbar-chip topbar-chip-warn"
                  title="Splits won't process until the bundled worker restarts. Restart StemStudio to relaunch it."
                  onClick={() =>
                    pushToast(
                      'error',
                      'Worker is offline. Restart StemStudio to resume splits.',
                    )
                  }
                >
                  <span className="topbar-dot topbar-dot-warn" />
...
function ConnectionDot({ connection, hasFirstSync }: { connection: Connection; hasFirstSync: boolean }) {
  ...
  useEffect(() => {
    if (connection.state !== 'offline' && connection.state !== 'error') return undefined
  ...
  if (connection.state === 'offline' || connection.state === 'error') {
    const retryInMs = connection.nextRetryAt ? connection.nextRetryAt - now : 0
    const retryIn = Math.max(0, Math.ceil(retryInMs / 1000))
    const label = connection.state === 'offline' ? 'offline' : 'sync error'
    const dotClass = connection.state === 'offline' ? 'topbar-dot-offline' : 'topbar-dot-warn'
    return (
      <span className="topbar-chip" title={connection.lastError ?? 'Connection error'}>
        <span className={`topbar-dot ${dotClass}`} />
        {label} · retry {retryIn}s
      </span>
    )
  }
  if (!hasFirstSync) {
