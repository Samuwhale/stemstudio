...
type ConnectionState = 'ready' | 'syncing' | 'offline' | 'error'
...
function failedDiagnostics(error: unknown): Diagnostics {
  return {
    app_ready: false,
    separation_ready: false,
    acceleration: 'unknown',
    free_disk_gb: 0,
    binaries: [],
    issues: [`Readiness checks failed: ${getErrorMessage(error)}`],
    data_directories: {},
    url_import_ready: false,
  }
}
...
function describeBatchCreateRunToast(results: PromiseSettledResult<RunMutationResponse>[]) {
  ...
  if (failed === results.length) {
    return {
      tone: 'error' as const,
      message: getErrorMessage(firstFailure?.reason),
    }
  }
  ...
  if (failed > 0) parts.push(`${failed} failed: ${getErrorMessage(firstFailure?.reason)}`)

  return {
    tone: failed > 0 ? ('error' as const) : ('success' as const),
    message: `${parts.join(' · ')}.`,
  }
}
          ...
          try {
            ...
            setDiagnostics(
              diagnosticsResult.status === 'fulfilled'
                ...
                : failedDiagnostics(diagnosticsResult.reason),
            )
            setSettings(nextSettings)
            setStorageOverview(storageOverviewResult.status === 'fulfilled' ? storageOverviewResult.value : null)
            setStorageOverviewError(
              storageOverviewResult.status === 'fulfilled'
                ? null
                : getErrorMessage(storageOverviewResult.reason),
            )
            ...
            setConnection((current) => {
              ...
              return {
                state: isApiError(error) || isApiConfigurationError(error) ? 'error' : 'offline',
  ...
  async function handleConfirmDrafts(
    ...
    try {
      ...
      const reusedMsg = result.reused_track_count
        ...
        : ''
      const skippedMsg = result.skipped_draft_count
        ? `${result.skipped_draft_count} skipped`
        : ''
      const queuedMsg = result.queued_run_count
        ? `${result.queued_run_count} stem set${result.queued_run_count === 1 ? '' : 's'} queued`
        : ''
      const parts = [createdMsg, reusedMsg, skippedMsg, queuedMsg].filter(Boolean)
      pushToast('success', `${result.tracks.length > 0 ? 'Imported' : 'Updated imports'}: ${parts.join(' · ')}.`)
      await refreshDashboard()
      return result
    } catch (error) {
      pushToast('error', getErrorMessage(error))
