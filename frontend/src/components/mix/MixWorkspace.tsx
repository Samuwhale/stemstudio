...
function sameMixSaveStatus(left: MixSaveStatus, right: MixSaveStatus) {
  return left.state === right.state && left.dirty === right.dirty && left.error === right.error
}
...
function StemSetsPopover({
  ...
  function stateLabel(run: RunDetail): string {
    ...
    if (RETRYABLE_STATUSES.has(run.status)) {
      if (!stemCreationReady) return 'Retry unavailable'
        ...
        {runs.length > 0 ? (
          <div className="popover-list" role="list">
            {runs.map((run) => {
              ...
              const retryUnavailable = RETRYABLE_STATUSES.has(run.status) && !stemCreationReady
              const disabled = RETRYABLE_STATUSES.has(run.status) && (retryingRunId === run.id || retryUnavailable)
              return (
                <button
                  key={run.id}
                  type="button"
                  ...
                  disabled={disabled}
                  title={retryUnavailable ? stemCreationBlockedReason : undefined}
                  onClick={() => {
...
function MixWorkspaceContent({
  ...
  const handleMixSaveStatusChange = useCallback((status: MixSaveStatus) => {
    setMixSaveState((current) => {
      if (current.runId === selectedRunStateId && sameMixSaveStatus(current.status, status)) {
        return current
      }
      return { runId: selectedRunStateId, status }
    })
  }, [selectedRunStateId, setMixSaveState])
              ...
              <>
                ...
                <div className="mix-blocked-actions">
                  <button
                    type="button"
                    className="button-primary"
                    disabled={retryingRunId === selectedRun.id || !stemCreationReady}
                    title={!stemCreationReady ? stemCreationBlockedReason : undefined}
                    onClick={() =>
                      discardRejection(async () => {
