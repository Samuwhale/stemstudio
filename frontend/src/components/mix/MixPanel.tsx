...
function equivalentMixEntries(left: RunMixStemEntry[], right: RunMixStemEntry[]) {
  const leftById = new Map(left.map((entry) => [entry.artifact_id, entry]))
  const rightById = new Map(right.map((entry) => [entry.artifact_id, entry]))
  const ids = new Set([...leftById.keys(), ...rightById.keys()])
  for (const id of ids) {
    const leftEntry = leftById.get(id)
    const rightEntry = rightById.get(id)
    const leftGain = leftEntry?.gain_db ?? 0
    const rightGain = rightEntry?.gain_db ?? 0
    const leftMuted = leftEntry?.muted ?? false
    const rightMuted = rightEntry?.muted ?? false
    if (Math.abs(leftGain - rightGain) > 0.01) return false
    if (leftMuted !== rightMuted) return false
  }
  return true
}
...
export function MixPanel({ run, onSave, saving, onSaveStatusChange }: MixPanelProps) {
  ...
  const locallySavedMixRef = useRef<RunMixStemEntry[] | null>(null)
  const tearingDownRef = useRef(false)
  const onSaveRef = useRef(onSave)
  const [retryPayload, setRetryPayload] = useState<RunMixStemEntry[] | null>(null)
  const [persistedMixStems, setPersistedMixStems] = useState<RunMixStemEntry[]>(() => run.mix.stems)
  ...
  useEffect(() => {
    if (saveState !== 'idle' || pendingSavePayloadRef.current) return
    const locallySavedMix = locallySavedMixRef.current
    if (locallySavedMix) {
      if (!equivalentMixEntries(run.mix.stems, locallySavedMix)) return
      locallySavedMixRef.current = null
    }
    setPersistedMixStems(run.mix.stems)
  }, [run.mix.stems, saveState])
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (pendingTimer !== null && pendingPayload) {
        discardRejection(() => onSaveRef.current(pendingPayload))
      }
    }
  }, [])
  ...
  const dirty = !equalsPersisted(stems, persistedMixStems)
  ...
  const persistMix = useCallback(
    async (payload: RunMixStemEntry[], revision = saveRevisionRef.current) => {
      ...
      try {
        ...
        if (tearingDownRef.current || latestSaveIdRef.current !== saveId || saveRevisionRef.current !== revision) return
        pendingSavePayloadRef.current = null
        setRetryPayload(null)
        locallySavedMixRef.current = payload
        setPersistedMixStems(payload)
        setSaveState('saved')
      } catch (error) {
        if (tearingDownRef.current || latestSaveIdRef.current !== saveId || saveRevisionRef.current !== revision) return
        setRetryPayload(payload)
  ...
  function scheduleSave(next: StemRow[]) {
    ...
    saveTimerRef.current = window.setTimeout(() => {
      saveTimerRef.current = null
      discardRejection(() => persistMix(payload, revision))
    }, SAVE_DEBOUNCE_MS)
  }
