...
import {
  filterImportableMediaFiles,
  IMPORTABLE_MEDIA_ACCEPT,
  IMPORTABLE_MEDIA_HINT,
  unsupportedImportableMediaMessage,
} from '../../importableMedia'
...
function ImportPanelContent({
  ...
  async function resolveYouTubeSource(url: string) {
    ...
    try {
      ...
      setSourceMessage({ tone: 'error', text: raw instanceof Error ? raw.message : 'Could not resolve URL.' })
    } finally {
  ...
  async function resolveLocalFiles(files: File[], ignoredFileCount = 0) {
    ...
    try {
      ...
      if (ignoredFileCount > 0) {
        setSourceMessage({ tone: 'notice', text: unsupportedImportableMediaMessage(ignoredFileCount) })
      }
    } catch (raw) {
      setLocalFiles([])
      if (fileInputRef.current) fileInputRef.current.value = ''
      setSourceMessage({ tone: 'error', text: raw instanceof Error ? raw.message : 'Could not stage those files.' })
    }
  }
  ...
  function handleDrop(event: DragEvent<HTMLDivElement>) {
    ...
    const accepted = filterImportableMediaFiles(droppedFiles)
    const ignoredFileCount = droppedFiles.length - accepted.length
    if (accepted.length === 0) {
      setSourceMessage({ tone: 'error', text: `Drop supported audio or video files. ${IMPORTABLE_MEDIA_HINT}` })
      return
    }
    setSourceMessage(null)
    discardRejection(() => resolveLocalFiles(accepted, ignoredFileCount))
  }
  ...
  const hasImportableDrafts = importableCount > 0
  const canConfirm = (
    drafts.length > 0
    && unresolved === 0
    && !confirming
  ...
  const canConfirmAndQueue = canConfirm && hasImportableDrafts && stemCreationReady && selection.stems.length > 0
  const selectedStemLabel = stemSelectionLabel(selection.stems, stemOptions)
  const canClose = !sourceBusy && !confirming && !hasPendingDraftActions
    ...
    <>
        ...
        <div className="import-panel-url-wrap">
          <input
            ...
            onChange={(event) => {
              ...
              if (sourceMessage) setSourceMessage(null)
            }}
            onKeyDown={(event) => {
      ...
      >
        <input
          ref={fileInputRef}
          type="file"
          accept={IMPORTABLE_MEDIA_ACCEPT}
          multiple
          disabled={sourceBusy}
          onChange={(event) => {
            const selectedFiles = Array.from(event.target.files ?? [])
            const accepted = filterImportableMediaFiles(selectedFiles)
            const ignoredFileCount = selectedFiles.length - accepted.length
            if (accepted.length > 0) {
              discardRejection(() => resolveLocalFiles(accepted, ignoredFileCount))
              return
            }
            setLocalFiles([])
            setSourceMessage({ tone: 'error', text: `Choose supported audio or video files. ${IMPORTABLE_MEDIA_HINT}` })
          }}
          hidden
        />
        {localFiles.length > 0 ? (
      ...
      {sourceMessage ? (
        <p className={`import-panel-message is-${sourceMessage.tone}`} role={sourceMessage.tone === 'error' ? 'alert' : 'status'}>
          {sourceMessage.text}
        </p>
      ) : null}
    </>
    ...
    >
      <div className="overlay-panel" ref={panelRef} tabIndex={-1}>
        <header className="overlay-head">
          ...
          <button ref={closeButtonRef} type="button" className="button-secondary" disabled={!canClose} onClick={onClose}>
            Close
          </button>
        </header>
            ...
            <div className="overlay-foot-bottom">
              <div className="overlay-foot-copy">
                {sourceBusy
                  ...
                  : unresolved > 0
                    ...
                    : !stemCreationReady
                      ? `Import now, then ${stemCreationBlockedReason.charAt(0).toLowerCase()}${stemCreationBlockedReason.slice(1)}`
              ...
              <div className="overlay-foot-actions">
                ...
                <button
                  ...
                  title={
                    hasImportableDrafts
                      ? 'Add to the library without processing yet. You can create a stem set from the song list.'
                      : 'Remove the skipped items from this review.'
                  }
                >
                  {hasImportableDrafts ? 'Import only' : 'Discard skipped'}
                </button>
              </div>
            </div>
          </footer>
